...
export function ProfileCard({ profile, isAdmin, isFavorited, onPress, onEdit, onDelete, onFavorite }: Props) {
  ...
  return (
    ...
    >
      ...
      <View style={s.info}>
        ...
        <Text style={s.meta} numberOfLines={1}>{profile.company?.name ?? '—'} · {profile.line?.name ?? '—'}</Text>
      ...
      {isAdmin ? (
        ...
        <View style={s.rightGroup}>
          ...
          {onFavorite && (
            ...
            >
              <Text style={[s.favIc, isFavorited && s.favIcActive]}>
                {isFavorited ? '❤️' : '🤍'}
              </Text>
            </TouchableOpacity>
          )}
...
const s = StyleSheet.create({
  ...
  favBtn:       { padding: 2 },
  favIc:        { fontSize: 17, opacity: 0.5 },
  favIcActive:  { opacity: 1 },
